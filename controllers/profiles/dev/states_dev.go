...
func (e *ensureRunningWorkflowState) Do(ctx context.Context, workflow *operatorapi.SonataFlow) (ctrl.Result, []client.Object, error) {
	...
	managedPropsCM, _, err := e.ensurers.managedPropsConfigMap.Ensure(ctx, workflow, pl, common.ManagedPropertiesMutateVisitor(ctx, e.C, e.Catalog, workflow, pl, userPropsCM.(*corev1.ConfigMap)))
	if err != nil {
		return ctrl.Result{Requeue: false}, objs, err
	}
	...
	deployment, _, err := e.ensurers.deployment.Ensure(ctx, workflow, pl,
		deploymentMutateVisitor(workflow, pl, e.StateSupport),
	...
	service, _, err := e.ensurers.service.Ensure(ctx, workflow, common.ServiceMutateVisitor(workflow, e.StateSupport))
	if err != nil {
		return ctrl.Result{RequeueAfter: constants.RequeueAfterFailure}, objs, err
	}
